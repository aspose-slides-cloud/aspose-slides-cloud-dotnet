...
namespace Aspose.Slides.Cloud.Live.Demos.UI.Config
{
    ...
    public class GlobalAppHelper
    {
		...
		public GlobalAppHelper(HttpContext hc, HttpApplicationState appState, string SessionID, string language)
		{
			...
			AsposeSlidesCloudContext.atcc = new AsposeSlidesCloudContext(hc); // sync the context
		...
		private void initResources(string ResourcesFile, string SessionID)
		{
			...
			if (AsposeSlidesCloudContext.Current.Cache[SessionID] == null)
			{
				...
				AsposeSlidesCloudContext.Current.Cache.Add(
				   SessionID,
					resources,
